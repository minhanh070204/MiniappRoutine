...
const pathHtml = "c:/Users/Le Minh Anh/Downloads/Minapp_Routine/bo-suu-tap-theo-mau-main/index.html";
...
const vip_card_loyalty = `                <div class="vip-card mb-8">
...
html = html.replace(/<div class="vip-card mb-8">[\s\S]+?(?=<\/div>\s*<\/div>\s*<!-- VIEW: CỬA HÀNG -->)/, vip_card_loyalty);

const offers_view = `<!-- VIEW: OFFERS -->
...
html = html.replace(/<!-- VIEW: CỬA HÀNG -->[\s\S]+?(?=<!-- VIEW: CÁ NHÂN -->)/, offers_view + '\\n\\n        ');
...
html = html.replace(/<nav class="bottom-nav">[\s\S]+?<\/nav>/, nav_html);

const store_overlay = `        <!-- STORE SEARCH OVERLAY -->
        <div id="overlay-store-search" class="overlay-view hidden">
            ...
            <div class="overlay-content p-4">
                <div class="card mb-6" style="background: var(--accent); color: white; padding: 24px;">
                    ...
                    <button class="btn border-white border text-white" style="border-width: 1px" onclick="openBooking('Routine Nguyễn Trãi')">ĐẶT LỊCH THỬ ĐỒ</button>
                </div>
                <div class="tabs mb-4">
        ...
        <div id="overlay-point-success" class="overlay-view hidden justify-center items-center p-6 text-center text-black" style="background: rgba(0,0,0,0.5); z-index: 1000;">
             <div class="card p-6 w-full" style="max-width:320px; margin: auto;">
                 <h2 class="text-xl font-bold mb-2">Chúc mừng bạn</h2>
                 <p class="text-sm mb-4" id="point-success-text">đã nhận Voucher mua sắm</p>
                 <div class="bg-white p-3 rounded mb-4 text-left" style="background:#f4f4f5">
                     <p class="text-xs text-gray"><b>ĐK Sử dụng:</b> Áp dụng hóa đơn nguyên giá</p>
                     <p class="text-xs text-gray"><b>HSD:</b> 30 ngày kể từ ngày quy đổi</p>
                 </div>
                 <button class="btn" onclick="closeOverlay('point-success');">ĐÓNG</button>
             </div>
        </div>
`;
...
const new_policies = `            <div class="overlay-content p-6 text-sm text-gray leading-relaxed">
...
html = html.replace(/<div class="overlay-content p-6 text-sm text-gray leading-relaxed">[\s\S]+?<\/div>/, new_policies);

fs.writeFileSync(pathHtml, html);

const pathJs = "c:/Users/Le Minh Anh/Downloads/Minapp_Routine/bo-suu-tap-theo-mau-main/main.js";
...
const rt_logic = `function renderTier(level) {
...
js = js.replace(/function renderTier\(level\) \{[\s\S]+?\}\s*\n\s*\}/, rt_logic);
